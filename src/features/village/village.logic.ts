...
export const createNPCSystem = () => {
  return {
    ...
    execute(
      ...
      // 1. Costs
      const costs =
        step.costs || (step.cost && step.costType ? { [step.costType]: step.cost } : null);
    ...
    canAccessTreeOfLife(game: GameState): boolean {
      return game.unlockedNPCs.includes('npc-treeOfLife' as import('../../types/game').NPCId) && !(game as any).demoCompleted;
    },
  };
};
